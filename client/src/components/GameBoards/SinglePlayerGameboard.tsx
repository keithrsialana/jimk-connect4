...
const SinglePlayerGameBoard: React.FC = () => {
		...
		if (player1Profile && player2Profile) {
			...
			const updateStats = async (stats: any, playerNumber: any) => {
				try {
					...
					console.error(
						`Error updating Player ${playerNumber}: ${error.graphQLErrors[0]?.message || error.message
						}`
					);
				}
			};
	...
	useEffect(() => {
		...
		if (player1 && player2 != "") {
			if (player1 != player2) setGameStartButton(true);
		} else if (player2 == "") setGameStartButton(false);
					...
					<div>
						<h1 className="current-move">
							...
							<span className={currentPlayer.toLowerCase()}>
								{currentPlayer == "Red" ? player1 : player2}
							</span>
						</h1>
					</div>
					<div className="game-container">
						<div className="in-game-profile-p1">
							<div className="card-title-p1">
								<h3 className="igp-card-items">
									{player1Profile
										? player1Profile.username
										: "player 1 profile not found"}
								</h3>
							</div>
							<p className="igp-card-items">
								Games Played:{" "}
								{player1Profile ? player1Profile.games_played : ""}
							</p>
							<p className="igp-card-items">
								Wins: {player1Profile ? player1Profile.games_won : ""}
							</p>
							<p className="igp-card-items">
								Losses: {player1Profile ? player1Profile.games_lost : ""}
							</p>
						</div>
						<div>
								...
								<div className="card-title-p2">
									<h3 className="igp-card-items">
										{player2Profile
											...
											: "player 2 profile not found"}
									</h3>
								</div>
								<p className="igp-card-items">
									Games Played:{" "}
									{player2Profile ? player2Profile.games_played : ""}
								</p>
								<p className="igp-card-items">
									Wins: {player2Profile ? player2Profile.games_won : ""}
								</p>
								<p className="igp-card-items">
									Losses: {player2Profile ? player2Profile.games_lost : ""}
								</p>
